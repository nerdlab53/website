...
export class Sign extends Phaser.GameObjects.Zone
{
	constructor(scene, x, y, text, direction) {
		...
		this.signText = pxText(scene, x+offsetX, y+offsetY, text, 8, PAL_HEX.navy, 'center')
			.setOrigin(0.5, 1)
			.setDepth(101)
			.setVisible(false);
		this.signRect = scene.add.rectangle(Math.round(x+offsetX), Math.round(y+offsetY), this.signText.width+12, this.signText.height+4, PAL.cream)
